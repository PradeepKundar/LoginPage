...
function pswrdReducer(state, action) {
  if (action.type === "USER_INPUT") {
...
const Login = (props) => {
  ...
  const [pswrdState, dispatchPswrd] = useReducer(pswrdReducer, {
    value: "",
    isValid: null,
  });
  ...
  const { isValid: pswrdIsValid } = pswrdState;
  // console.log(emailIsValid);
  // console.log(pswrdIsValid);
  ...
  useEffect(() => {
    const validation = setTimeout(() => {
      ...
      setFormIsValid(emailIsValid && pswrdIsValid);
    }, 500);
